...
class GeolocationRequestViewSetListTest(APITestCase):
    def test_cannot_list_as_unauthenticated_user(self):
        response = self.client.get(reverse('geolocations:geolocation-list'))
        self.assertEqual(response.status_code, 401)
        self.assertDictEqual(response.json(), {'detail': 'Authentication credentials were not provided.'})

    def test_list_own_geolocation_requests(self):
...
class GeolocationRequestViewSetCreateTest(APITestCase):
    def test_cannot_create_as_unauthenticated_user(self):
        ...
        response = self.client.post(reverse('geolocations:geolocation-list'), data=data)
        self.assertEqual(response.status_code, 401)
        self.assertDictEqual(response.json(), {'detail': 'Authentication credentials were not provided.'})
...
class GeolocationRequestViewSetDeleteTest(APITestCase):
    def test_cannot_delete_as_unauthenticated_user(self):
        ...
        response = self.client.delete(reverse('geolocations:geolocation-detail', kwargs=dict(pk=request.pk)))
    ...
    def test_delete_own_geolocation_request(self):
        user = UserFactory(is_active=True)
...
class GeolocationRequestViewSetRetrieveTest(APITestCase):
    def test_cannot_retrieve_as_unauthenticated_user(self):
        request = GeolocationRequestFactory()
        response = self.client.get(reverse('geolocations:geolocation-detail', kwargs=dict(pk=request.pk)))

        self.assertEqual(response.status_code, 401)
        self.assertDictEqual(response.json(), {'detail': 'Authentication credentials were not provided.'})

    def test_cannot_retrieve_someone_else_geolocation_request(self):
        request = GeolocationRequestFactory()
        user = UserFactory(is_active=True)
        self.client.force_authenticate(user=user)
    ...
    def test_retrieve_own_geolocation_request(self):
        user = UserFactory(is_active=True)
        request = GeolocationRequestFactory(user=user)
        self.client.force_authenticate(user=user)
